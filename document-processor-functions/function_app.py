...
def save_to_blob(content: str, output_path: str, content_type: str = "application/json") -> str:
    ...
    try:
        ...
        blob_client.upload_blob(content, overwrite=True, content_settings=ContentSettings(
            content_type=content_type))
...
def extract_kv_tables(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Request body:
    { "extracted_file_path": "dev-mr-miyagi/outcome/CDS_test/CDS_test-extracted.markdown
    ...
    log_event(
        "api",
        "extract_kv_tables",
        "API 4: Extract KV and Tables - HTTP trigger called",
        status="info"
    )
    ...
    try:
        ...
        log_event(
            "api",
            "extract_kv_tables",
            f"Processing markdown file: {file_path}",
            status="info",
            file_name=file_path
        )
        ...
        if total_tokens > max_chunk_tokens:
            log_event(
                "api",
                "extract_kv_tables",
                "Content exceeds limit, chunking into smaller pieces",
                status="info",
                file_name=file_path
            )
        ...
        log_event(
            ...
            (
                ...
                f"{response_data['total_key_value_pairs']} key-value pairs "
                f"and {response_data['total_tables']} tables"
            ),
            status="success",
            file_name=file_path
        )
        ...
        log_event(
            ...
            "Extraction failed",
            status="error",
            file_name=req_body.get("file_path"),
            error_details=str(e)
        )
        return func.HttpResponse(
            json.dumps({"error": f"Processing error: {str(e)}"}),
